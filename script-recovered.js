...
const TORPEDO_TUBE_GEOMETRY = {
  "bow-1": { x: 28.0, y: -2.4, facing: 0 },
  "bow-2": { x: 28.0, y: -0.8, facing: 0 },
  "bow-3": { x: 28.0, y: 0.8, facing: 0 },
  "bow-4": { x: 28.0, y: 2.4, facing: 0 },
  "stern-1": { x: -24.0, y: 0.0, facing: 180 }
};
...
function createTDCState() {
  return {
    ...
    tubeId: null,
    tubeOffsetWorld: null,
    parallaxBearing: null,
    parallaxRange: null,
    runProfile: {
      initialStraightMeters: 9.5,
      turnRadiusMeters: 95,
      enabled: true
    }
  };
...
function getTubeGeometry(tubeId) {
  return tubeId ? TORPEDO_TUBE_GEOMETRY[tubeId] || null : null;
}

function getTubeWorldPosition(tubeId, sub = state.submarine) {
  const geometry = getTubeGeometry(tubeId);
  if (!geometry || !sub) return null;
  const headingRad = toRadians(sub.heading);
  const dx =
    geometry.x * Math.cos(headingRad) -
    geometry.y * Math.sin(headingRad);
  const dy =
    geometry.x * Math.sin(headingRad) +
    geometry.y * Math.cos(headingRad);
  return {
    tubeId,
    x: sub.x + dx,
    y: sub.y + dy,
    localX: geometry.x,
    localY: geometry.y,
    facing: normalizeAngle(sub.heading + geometry.facing),
    arc: geometry.facing === 180 ? "stern" : "bow"
  };
}

function syncTDCLaunchGeometry(tubeId = state.torpedoSequence.selectedTubeId, sub = state.submarine) {
  state.tdc.tubeId = tubeId || null;
  state.tdc.tubeOffsetWorld = tubeId ? getTubeWorldPosition(tubeId, sub) : null;
}

function formatTubeGeometrySummary(tubeId = state.tdc.tubeId, sub = state.submarine) {
...
function computeTDCSolution() {
  const tdc = state.tdc;
  const torpedo = getActiveTorpedoSpec();
  syncTDCLaunchGeometry(state.torpedoSequence.selectedTubeId || tdc.tubeId);
  ...
  if (tdcSolutionNoteNode) {
    ...
    if (solution) {
      ...
      tdcSolutionNoteNode.textContent =
        ...
        ` 速度/AOB が固まると会敵時間を表示。 / ${formatTubeGeometrySummary()}`;
    } else {
      tdcSolutionNoteNode.textContent = `会敵時間と予想到達点をここに表示。 / ${formatTubeGeometrySummary()}`;
    }
  }
}
